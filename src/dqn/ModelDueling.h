...
namespace dqn
{
	...
	private:
		...
		void get_gradient_from_branch(const std::vector<nn::Layer*>& branch, std::map<const nn::Layer*, xt::xarray<float>>& tape,
		...
		xt::xarray<float> call_with_tape(xt::xarray<float>& state, xt::xarray<float>& actions,
			std::map<const nn::Layer*, xt::xarray<float>>* tape) const override;

	public:
		ModelDueling();
		void build(std::vector<std::size_t> input_shape) const override;
		xt::xarray<xt::xarray<float>> get_gradient(std::map<const nn::Layer*,
